...
import { get_EddsaSig_ExitAmmPool, getAmmExitEcdsaTypedData } from '../../../index';
...
describe("amm_calc", function () {
  ...
  it("getEddsaSigExitAmmPool", async () => {
      const response = await LoopringAPI.ammpoolAPI.getAmmPoolSnapshot<any>({
        poolAddress: TOKEN_INFO.tokenMap[ "LP-LRC-ETH" ].address,
      });
      console.log(response.raw_data.pooled);
      ...
      const res = {...request, validUntil: LOOPRING_EXPORTED_ACCOUNT.validUntil}

      get_EddsaSig_ExitAmmPool(res, patch)
      get_EddsaSig_ExitAmmPool({
        ...res,
        domainSeparator: "0x68561fa99d9c0532544589587b389976733591df662e3ca157dfdd30db83e966"
      }, patch)
    },
    DEFAULT_TIMEOUT
  );

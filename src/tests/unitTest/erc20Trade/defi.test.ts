...
describe('DefiAPI test', function () {
  it(
    ...
    async () => {
      ...
      console.log('getDefiMarkets', response.marketArr, response.markets, response.pairs)
    },
    DEFAULT_TIMEOUT,
  )
  ...
  it(
    ...
    async () => {
      ...
      console.log('getDefiToken:', response.tokensMap, response.idIndex, response.addressIndex)
    },
    DEFAULT_TIMEOUT,
  )
  it(
  ...
  it(
    ...
    async () => {
      ...
      const { fees } = await LoopringAPI.userAPI.getOffchainFeeAmt(
        {
          accountId: accInfo.accountId,
          requestType: isJoin ? sdk.OffchainFeeReqType.DEFI_JOIN : sdk.OffchainFeeReqType.DEFI_EXIT,
          market: testMarket,
        },
        apiKey,
      )
      ...
      console.log('calcVol', calcVol, calcVol.miniSellVol, calcVol.maxSellVol, calcVol.maxFeeBips)
      ...
      const response = await LoopringAPI.defiAPI.orderDefi(request, eddsaKey.sk, apiKey)

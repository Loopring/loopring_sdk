import { IsMobile, LOOPRING_URLs, LoopringErrorCode, ReqMethod } from '../defs'
...
import { LOOPRING_EXPORTED_ACCOUNT, web3 } from './test.MockData'
...
describe('formatter test', function () {
  ...
  it(
    ...
    async () => {
      const px = fm.formatEddsaKey(
        fm.toHex(
          fm.toBig('4966334643155205117396087046889763297010019574817548934733242435404623801022'),
        ),
      )
      const py = fm.formatEddsaKey(
        fm.toHex(
          fm.toBig('12336121800785994591019936445923899190012791232373302800087997800255990054103'),
        ),
      )
  ...
  it('test padLeftEven', async () => {
    expect(padLeftEven(LOOPRING_EXPORTED_ACCOUNT.address)).toBe(LOOPRING_EXPORTED_ACCOUNT.address)
  ...
  it('test formatKey', async () => {
    ...
    expect(formatKey(LOOPRING_EXPORTED_ACCOUNT.testNotOx)).toBe(LOOPRING_EXPORTED_ACCOUNT.testNotOx)
  })

  // Missing test input data for Uint8Array
  it('test formatAddress', async () => {
    expect(formatAddress(LOOPRING_EXPORTED_ACCOUNT.testNotOx).toLowerCase()).toBe(
  ...
  it('test addHexPrefix', async () => {
    expect(addHexPrefix(LOOPRING_EXPORTED_ACCOUNT.address)).toBe(LOOPRING_EXPORTED_ACCOUNT.address)
  ...
  it('test formatEddsaKey', async () => {
    expect(formatEddsaKey('027a5b716c7309a30703ede3f1a218cdec857e424a31543f8a658e7d2208db33')).toBe(

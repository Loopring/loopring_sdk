...
describe("AmmpoolAPI test", function () {
  ...
  it(
    ...
    async () => {
      ...
      const response: any = await api.getAmmPoolGameRank({
  ...
  it(
    ...
    async () => {
      const response = await api.getAmmPoolGameUserRank(
        { owner: testAddress, ammPoolMarket: "LRC-ETH" },
        acc.apiKey
      );
  ...
  it(
    "getAmmPoolActivityRules_Mainnet",
    async () => {
      api = new AmmpoolAPI({ chainId: ChainId.MAINNET });
      ...
      console.log(
        ...
        response.groupByRuleTypeAndStatus.ORDERBOOK_MINING
      );
    },
    DEFAULT_TIMEOUT
  );
  ...
  it(
    ...
    async () => {
      try {
        ...
        const storageId_1 = await userApi.getNextStorageId(
          request_1,
          acc.apiKey
        );
        ...
        const { fees } = await userApi.getOffchainFeeAmt(
          {
            tokenSymbol: "ETH",

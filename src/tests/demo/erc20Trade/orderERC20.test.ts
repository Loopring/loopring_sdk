...
const calculateSwap = (
  ...
  if (depth && market && tokenMap) {
    ...
    if (amountMap && amountMap[market] && ammMap) {
      ...
      feeTakerRate = amountMarket[buySymbol] && amountMarket[buySymbol].userOrderInfo.takerRate
       ...
       */
      minAmt = BigNumber.max(sellToken.orderAmounts.dust, calcForMinCost?.amountS ?? 0).times(1.1)
    ...
    if (
       ...
       * totalFee
       */
      totalFee = totalFeeRaw.div('1e' + tokenMap[minSymbol].decimals).toString()
      /** @output: UI
...
describe('orderERC20', function () {
  ...
  it('Swap_SellETH_BuyLRC_USER_INPUT_ETH', async () => {
    ...
    const { calcTradeParams, maxFeeBips, minimumReceived } =
      calculateSwap(
        'ETH',
        'LRC',
        false,
        inputValue, // user Input value no decimal 10 lrc,
        slippage,
      ) ?? {}
    ...
    console.log('eddsaKey:', eddsaKey.sk, {
      ...
      maxFeeBips: maxFeeBips ?? 5,
      fillAmountBOrS: false, // amm only false
      tradeChannel: calcTradeParams?.exceedDepth ? sdk.TradeChannel.BLANK : sdk.TradeChannel.MIXED,
      orderType: calcTradeParams?.exceedDepth ? sdk.OrderType.ClassAmm : sdk.OrderType.TakerOnly,
      eddsaSignature: '',
    })
    const dataToSig = [
      ...
      calcTradeParams?.amountS as string,
      TOKEN_INFO.tokenMap[buy].tokenId,

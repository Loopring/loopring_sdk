...
import {
	ABInfo,
	AmmPoolSnapshot, CalDualResult,
	DefiMarketInfo,
	DepthData, DUAL_TYPE, DualBalance, DualIndex, DualProductAndPrice, DualRulesCoinsInfo,
	ExitAmmPoolRequest,
	JoinAmmPoolRequest,
	LoopringMap,
	MarketInfo,
...
export function calcDual(
	...
	const sellVol =
		fm.toBig(sellAmount ? sellAmount : 0)
	...
	const settleRatio = fm.toBig(info.dualPrice.dualBid[ 0 ].baseProfit).times(info.ratio).toFixed(Number(rule.baseProfitStep), BigNumber.ROUND_DOWN);
	if (info.dualType === DUAL_TYPE.DUAL_BASE) {
		...
		greaterEarnVol = toBig(toBig(settleRatio).plus(1).times(sellAmount ? sellAmount : 0)
			.times(info.strike).toFixed(buyToken.precision, BigNumber.ROUND_CEIL))
		...
		miniSellVol = BigNumber.max(dualMarket.baseLimitAmount, rule.baseMin); // rule.baseMin;
		quota = BigNumber.min(info.dualPrice.dualBid[ 0 ].baseQty, balance ? balance[ sellToken.symbol ]?.free : 0)
		...
		lessEarnVol = toBig(toBig(settleRatio).plus(1).times(sellAmount ? sellAmount : 0)
			// .times(1 + info.ratio)
			.div(info.strike).toFixed(buyToken.precision, BigNumber.ROUND_CEIL))
			.times("1e" + buyToken.decimals);
		...
		miniSellVol = BigNumber.max(dualMarket.quoteLimitAmount, rule.currencyMin); // rule.baseMin;
